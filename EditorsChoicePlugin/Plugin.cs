        ...
        if (Configuration.DoScriptInject) {
            if (!string.IsNullOrWhiteSpace(applicationPaths.WebPath))
            {
                ...
                if (File.Exists(indexFile))
                {
                    ...
                    string scriptElement = string.Format("<script plugin=\"EditorsChoice\" defer=\"defer\" version=\"1.0.0.0\" src=\"{0}/EditorsChoice/script\"></script><style plugin=\"EditorsChoice\">.sections.homeSectionsContainer {1}</style>", basePath, "{padding-top:360px;}");
